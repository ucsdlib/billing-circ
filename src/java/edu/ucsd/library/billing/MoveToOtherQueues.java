	...
	public void doPost(HttpServletRequest request,HttpServletResponse response){
		...
		if(foundSessionData && foundRequestData){
			try {
				...
				{
					deletedArray = deleteRecords(pending,temp);
					addedArray =addRecords(pending,problem,temp);
					results.put("pending", deletedArray);
					results.put("problem", addedArray);
					results.put("newPatron", newPatron);
					results.put("pendingTotal",deletedArray.size());
					...
					session.setAttribute("pendingData", deletedArray);
					session.setAttribute("newPatronData", newPatron);
					
				}
				else if (whichQueue.equals("Q"))
				{
					deletedArray = deleteRecords(problem,temp);
					addedArray =addRecords(problem,pending,temp);
					results.put("pending", addedArray);
					results.put("problem", deletedArray);
					results.put("newPatron", newPatron);
				...
				{
					deletedArray = deleteRecords(newPatron,temp);
					addedArray =addRecords(newPatron,pending,temp);
					results.put("pending", addedArray);
					results.put("problem", problem);
					results.put("newPatron", deletedArray);
					results.put("pendingTotal",addedArray.size());
					results.put("problemTotal",problem.size());
					results.put("newPatronTotal",deletedArray.size());
					session.setAttribute("problemData", problem);
					session.setAttribute("pendingData", addedArray);
					session.setAttribute("newPatronData", deletedArray);
		...
		for(int i=0; i < from.size();i++)
		  {
			 flag = false;
			  ...
			  for(int j=0; j<tokencount;j++)
			  {
				  ...
				  String invNo = arr[0];
				  String chargeType = arr[1];
				  log.info("$$$$ UUUUUUUUUU addRecords UUUUUUUUUUUUUUUU");
				  log.info("$$$$ INVno: "+invNo);
				  log.info("$$$$ ChargeType:"+chargeType);
				  //===============================
				  if(((obj.get("invoiceNo")).equals(invNo)) && ((obj.get("chargeType")).equals(chargeType)))
